...
import { getServiceUsage, type ServiceUsageGuide } from "../lib/service-usage";
...
const layout = ref<MetricsLayoutItem[]>(
  DEFAULT_METRICS_LAYOUT.map((x) => ({ ...x })),
);
...
const usageGuide = ref<ServiceUsageGuide | null>(null);
const usageService = ref<MetricsServiceStatus | null>(null);
...
const FIELD_HELP = {
  ...
  cpuCores:
    "逻辑 CPU 核数（含超线程）。对比负载时，负荷接近或超过核数通常表示较忙。",
  ...
  services:
    "基于监听端口识别常见服务（网站、数据库、缓存等）。未监听不等于服务未安装。",
...
function usageHost(): string {
  const host = props.hostId
    ? app.hosts.find((h) => h.id === props.hostId)
    : undefined;
  return (
    host?.host ||
    metrics.value?.primaryIp ||
    metrics.value?.hostname ||
    "主机IP"
  );
}

function openServiceUsage(s: MetricsServiceStatus): void {
  const hostCfg = props.hostId
    ? app.hosts.find((h) => h.id === props.hostId)
    : undefined;
  usageService.value = s;
  usageGuide.value = getServiceUsage(s, {
    host: usageHost(),
    username: hostCfg?.username,
  });
}

function closeServiceUsage(): void {
  usageGuide.value = null;
  usageService.value = null;
}

function onUsageKeydown(e: KeyboardEvent): void {
  if (e.key === "Escape") closeServiceUsage();
}
...
const filteredServices = computed(() => {
  ...
  if (serviceFilter.value === "running")
    return list.filter((s) => s.status === "running");
  if (serviceFilter.value === "stopped")
    return list.filter((s) => s.status === "stopped");
  return list;
});
...
const servicesPageCount = computed(() =>
  pageCount(filteredServices.value.length),
);
const pagedServices = computed(() =>
  pageSlice(filteredServices.value, servicesPage.value),
);
...
const portsTcpCount = computed(
  () => allPorts.value.filter((p) => p.protocol === "tcp").length,
);
const portsLocalCount = computed(
  () => allPorts.value.filter((p) => isLocalListenAddress(p.address)).length,
);
const portsPublicCount = computed(() =>
  Math.max(0, portsTotal.value - portsLocalCount.value),
);
const portsNamedCount = computed(
  () => allPorts.value.filter((p) => !!p.process?.trim()).length,
);
const portsAnonCount = computed(() =>
  Math.max(0, portsTotal.value - portsNamedCount.value),
);
...
const pagedPorts = computed(() =>
  pageSlice(filteredPorts.value, portsPage.value),
);
...
const pagedProcesses = computed(() =>
  pageSlice(sortedProcesses.value, procPage.value),
);
...
const pagedDisks = computed(() =>
  pageSlice(metrics.value?.disks || [], disksPage.value),
);
...
function shiftPage(
  ...
  if (which === "proc")
    procPage.value = clampPage(procPage.value + delta, procPageCount.value);
  else if (which === "ports")
    portsPage.value = clampPage(portsPage.value + delta, portsPageCount.value);
  else if (which === "services")
    servicesPage.value = clampPage(
      servicesPage.value + delta,
      servicesPageCount.value,
    );
  else
    disksPage.value = clampPage(disksPage.value + delta, disksPageCount.value);
}
...
onMounted(() => {
  ...
  window.addEventListener("keydown", onUsageKeydown);
...
onUnmounted(() => {
  window.removeEventListener("keydown", onUsageKeydown);
  offData?.();
  void stop();
});
</script>
...
<template>
  <div class="metrics" :class="{ disabled }">
    ...
    <div
      v-if="metrics"
      class="layout-grid"
      :class="{ 'layout-stacked': useStackedSide }"
    >
      <section
        ...
        <div class="card-body">
          <template v-if="item.id === 'system'">
            <div class="grid info">
              <div>
                <span class="label"
                  >主机名 <GlassTip :text="FIELD_HELP.hostname"
                /></span>
                <strong>{{ metrics.hostname }}</strong>
              </div>
              <div>
                <span class="label"
                  >系统 <GlassTip :text="FIELD_HELP.os"
                /></span>
                <strong>{{ metrics.os }}</strong>
              </div>
              <div>
                <span class="label"
                  >公网 IP <GlassTip :text="FIELD_HELP.primaryIp"
                /></span>
                <strong>{{ metrics.primaryIp || "-" }}</strong>
              </div>
              <div>
                <span class="label"
                  >网关 <GlassTip :text="FIELD_HELP.gateway"
                /></span>
                <strong>{{ metrics.gateway || "-" }}</strong>
              </div>
              <div>
                <span class="label"
                  >内核 <GlassTip :text="FIELD_HELP.kernel"
                /></span>
                <strong>{{ metrics.kernel }}</strong>
              </div>
              <div>
                <span class="label"
                  >架构 <GlassTip :text="FIELD_HELP.arch"
                /></span>
                <strong>{{ metrics.arch }}</strong>
              </div>
              <div>
                <span class="label"
                  >CPU 核数 <GlassTip :text="FIELD_HELP.cpuCores"
                /></span>
                <strong>{{ metrics.cpuCores || "-" }}</strong>
              </div>
              <div>
                <span class="label"
                  >CPU 型号 <GlassTip :text="FIELD_HELP.cpuModel"
                /></span>
                <strong>{{ metrics.cpuModel || "-" }}</strong>
              </div>
              <div>
                <span class="label"
                  >运行时间 <GlassTip :text="FIELD_HELP.uptime"
                /></span>
                <strong>{{ formatUptime(metrics.uptimeSec) }}</strong>
              </div>
              <div>
                <span class="label"
                  >服务器时间 <GlassTip :text="FIELD_HELP.serverTime"
                /></span>
                <strong>{{ metrics.serverTime || "-" }}</strong>
              </div>
              <div class="full">
                <span class="label"
                  >负载 <GlassTip :text="FIELD_HELP.load"
                /></span>
                <strong
                  >{{ metrics.load1.toFixed(2) }} /
                  {{ metrics.load5.toFixed(2) }} / {{ metrics.load15.toFixed(2)
          ...
          <template v-else-if="item.id === 'cpu'">
            <div class="ring-metric">
              <div
                class="ring"
                :class="`usage-${usageTone(metrics.cpuPercent)}`"
              >
              ...
              <p
                v-if="metrics.cpuModel && metrics.cpuModel !== '-'"
                class="sub"
              >
                {{ metrics.cpuModel
                }}<template v-if="metrics.cpuCores">
          ...
          <template v-else-if="item.id === 'mem'">
            <div class="ring-metric">
              ...
              >
                ...
                <div class="ring-center">
                  <strong>{{
                    pct(metrics.memUsed, metrics.memTotal).toFixed(1)
                  }}</strong>
                  <span>%</span>
                </div>
              </div>
              <p class="sub">
                ...
                {{ formatBytes(metrics.memTotal) }} · Swap
          ...
          <template v-else-if="item.id === 'gpu'">
            <div v-if="!metrics.gpus?.length" class="hint">
              未检测到 NVIDIA GPU（需 nvidia-smi）
            </div>
            <div
              v-for="g in metrics.gpus || []"
              :key="g.index"
              class="gpu-card"
            >
              <div class="gpu-top">
              ...
              <div class="gpu-stats">
                <div>
                  ...
                  <div class="bar">
                    <i :style="{ width: `${g.utilPercent}%` }" />
                  </div>
                </div>
                <div>
                  <span>显存</span>
                  <strong
                    >{{ g.memUsedMiB.toFixed(0) }} /
                    {{ g.memTotalMiB.toFixed(0) }} MiB</strong
                  >
                  <div class="bar">
                    <i
                      :style="{
              ...
              >
                <div class="disk-top">
                  ...
                  <span
                    ...
                    {{ formatBytes(d.used) }} / {{ formatBytes(d.size) }}</span
                  >
                </div>
                <div class="bar">
          ...
          <template v-else-if="item.id === 'net'">
            <div class="wave-wrap">
              ...
              <div v-if="netHistory.length < 2" class="wave-empty">
                采集中，波形即将出现…
              </div>
            </div>
            ...
            <div v-else class="table-scroll">
              <div class="table net-table">
                <div class="thead">
                  <span class="net-name">网卡</span>
                  <span class="net-num">下行</span>
                  <span class="net-num">上行</span>
                  <span class="net-num">累计 RX</span>
                  <span class="net-num">累计 TX</span>
                </div>
                <div v-for="n in metrics.nets" :key="n.name" class="trow">
                  <span class="net-name">{{ n.name }}</span>
                  <span class="net-num">{{ formatRate(n.rxRate) }}</span>
                  <span class="net-num">{{ formatRate(n.txRate) }}</span>
                  <span class="net-num">{{ formatBytes(n.rxBytes) }}</span>
                  <span class="net-num">{{ formatBytes(n.txBytes) }}</span>
                </div>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="item.id === 'services'">
            ...
            <div class="svc-grid">
              <button
                ...
                class="svc-item"
                :class="s.status"
                :title="'点击查看如何使用 ' + s.name"
                @click="openServiceUsage(s)"
              >
                <div class="svc-top">
          ...
          <template v-else-if="item.id === 'ports'">
            <div v-if="!portsTotal" class="hint">
              暂无监听端口数据（可能无 ss/权限）
            </div>
            <template v-else>
              <div class="list-head">
              ...
              <div class="table-scroll">
                <div class="table port-table">
                  <div class="thead">
                    <span>协议</span><span>端口</span><span>提示</span
                    ><span>地址</span><span>进程</span>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <span class="port-hint-cell">
                      <span
                        v-if="hintForPort(p.port)"
                        class="port-hint-chip sm"
                        >{{ hintForPort(p.port) }}</span
                      >
    ...
    <Teleport to="body">
      <div v-if="usageGuide" class="usage-mask" @click.self="closeServiceUsage">
        <div class="usage-modal glass-panel" role="dialog" aria-modal="true">
          <div class="usage-inner">
            <header>
              <div class="usage-title">
                <h3>{{ usageGuide.title }}</h3>
                <span
                  v-if="usageService"
                  class="usage-status"
                  :class="usageService.status"
                >
                  {{
                    usageService.status === "running" ? "运行中" : "未检测到"
                  }}
                </span>
              </div>
              <button
                ...
                class="btn-icon"
                aria-label="关闭"
                @click="closeServiceUsage"
              >
                ×
              </button>
            </header>
            <p class="usage-summary">{{ usageGuide.summary }}</p>
            <ol class="usage-steps">
              <li v-for="(step, i) in usageGuide.steps" :key="i">{{ step }}</li>
            </ol>
            <p v-if="usageGuide.tip" class="usage-tip">{{ usageGuide.tip }}</p>
...
.card[data-card="net"] .card-top {
  flex-wrap: wrap;
  gap: 8px;
}

.card[data-card="net"] .card-title-row {
  flex: 0 0 auto;
}

.card[data-card="net"] .net-tools {
  flex: 1 1 200px;
  min-width: 0;
  max-width: 100%;
...
.card[data-card="net"] .iface :deep(.glass-select) {
  min-width: 96px;
  max-width: 140px;
}
...
.port-table .thead,
.port-table .trow {
  grid-template-columns: 44px 56px minmax(64px, 0.7fr) minmax(0, 1.2fr) minmax(
      0,
      1fr
    );
}
...
.net-table .thead,
.net-table .trow {
  grid-template-columns: minmax(72px, 1.2fr) repeat(4, minmax(0, 1fr));
}
...
.svc-item.running:hover {
  border-color: rgba(125, 255, 168, 0.5);
...
.svc-dot.running {
  background: #7dffa8;
  box-shadow:
    0 0 0 1px rgba(125, 255, 168, 0.35),
    0 0 8px rgba(125, 255, 168, 0.35);
}
