...
个人自用的 Electron SSH / SFTP / 数据库工作台客户端（Windows）。
...
- 多连接标签、每连接内切换终端 / SFTP / 监控 / 数据库
- 按主机记录的命令历史（终端 / SFTP 自动收集）
- 数据库工作台：跟随当前 SSH 会话连接 MySQL/MariaDB、PostgreSQL
- 数据库结构浏览、SQL 选中执行、常用 SQL 示例、SQL 历史、CSV 导出、查询取消
- 数据库连接断开后，读取结构和安全查询支持自动重连
...
- **终端 | SFTP | 监控 | 数据库**：属于当前选中连接，切换视图不会断开连接。
...
- 在连接内切换到 **数据库** 标签，可基于当前 SSH 会话访问远端数据库。
- 当前版本支持 MySQL / MariaDB、PostgreSQL。
- 数据库连接通过 SSH `forwardOut` 建立，适合数据库只监听远端 `127.0.0.1` 或内网地址的场景。
- 左侧会检测当前服务器数据库服务和默认端口，状态分为 `服务 ✓/✕` 与 `端口 ✓/✕`；悬浮可查看探测依据。
- 数据库凭据保存在本地加密保险库中，列表不会展示密码。
- 结构区可浏览 database / schema / table / column。点击表会在 SQL 工作台下方追加安全预览语句，不会自动执行。
- **执行 SQL 时必须先选中要执行的语句**；未选中时不会执行，避免误跑整段脚本。
- 常用 SQL 示例、历史 SQL、点击表生成的 SQL 都会追加到编辑器下方，不会覆盖原内容。
- 支持轻量 SQL 高亮、格式化、关键字 / 表名 / 字段名补全。
- 查询结果支持 CSV 导出；长查询支持取消。
- 如果数据库连接被服务端关闭，读取结构和安全查询会自动重连并重试；写入 / DDL 不会自动重放，需重新确认执行。
...
- 数据库 SQL 历史按数据库凭据保存，和终端 / SFTP 命令历史分开管理。
...
| Ctrl + 滚轮  | 调节终端字号          |
| F9           | 数据库：执行选中 SQL  |
| Ctrl + Z     | 数据库：撤销 SQL 编辑 |
| Ctrl + Y     | 数据库：重做 SQL 编辑 |
| Ctrl + Space | 数据库：打开 SQL 补全 |

---

## 数据存储

主机、数据库凭据、命令历史、SQL 历史、书签、设置等保存在本机加密数据文件中，**不会**打入安装包。
...
- 数据库密码会加密保存；数据库连接列表和 UI 状态不会明文展示密码。
...
**Q：为什么数据库列表只显示少数几个库？**  
A：MySQL 的 `SHOW DATABASES` 会受当前数据库账号权限影响。请确认本软件和其它数据库软件使用的是同一个用户名、地址和端口；也可执行 `SHOW GRANTS FOR CURRENT_USER();` 查看权限。

**Q：执行 `SHOW TABLES;` 为什么提示 `No database selected`？**  
A：当前连接没有选定默认数据库。可先执行 `USE 数据库名;`，或使用 `SHOW TABLES FROM 数据库名;`，也可以在数据库凭据里填写“默认数据库”后重新连接。
